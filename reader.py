...
# Model
flags.DEFINE_string("model_config_path", default="/home/jovyan/MRC/Pipeline/Reader/xlnet_config.json",  ###############Model config path#############
...
flags.DEFINE_string("init_checkpoint", default="/workspace/jovyan/model.ckpt-15000",     ###############checkpoint path for initializing the model#############
...
flags.DEFINE_string("spiece_model_file", default="/home/jovyan/MRC/Pipeline/Reader/spiece.model",        ###############Sentence Piece model path#############
...
flags.DEFINE_integer("predict_batch_size", default=2,
                     help="batch size for prediction")
flags.DEFINE_integer("n_best_size", default=1,
                     help="n best size for predictions")
flags.DEFINE_integer("start_n_top", default=1, help="Beam size for span start.")
flags.DEFINE_integer("end_n_top", default=1, help="Beam size for span end.")
flags.DEFINE_string("target_eval_key", default="best_f1",
                    help="Use has_ans_f1 for Model I.")
flags.DEFINE_string("export_dir_base", default="/home/jovyan/exported-tf-model-1.15.2",          ###############Path of exported models#############
...
######################################################################################Read Test data###############################################################################################
####################################################################################Read Test data#####################################################################################################
def read_mrqa_data(input_data, is_training):
...
def mrqa_tester(context):
  """
  Get prediction with the data got fron mrqa official request.
  """
  eval_data = read_mrqa_data(context, is_training=False)
...
def get_ans():
    ...
    res = requests.get("http://0.0.0.0:9002/get_context", json={"questions":questions}) # put the question
    #res = get_contect(ques)
    if res.status_code==200:
        context = res.json()
    #print("CONTEXT: " , context)
    #step 2:
    #out = model(context, [1,23])
    out = mrqa_tester(context)
    #print("OUTTTTT: " , out)
